...
namespace TMPro.Examples
{
    public class VertexJitter : MonoBehaviour
    {
        ...
        private IEnumerator AnimateVertexColors()
        {
            ...
            {
                vertexAnim[i].angleRange = Random.Range(minInclusive: 10f, maxInclusive: 25f);
                vertexAnim[i].speed = Random.Range(minInclusive: 1f, maxInclusive: 3f);
            }
            ...
            while (true)
            {
                ...
                for (var i = 0; i < characterCount; i++)
                {
                    ...
                    vertAnim.angle = Mathf.SmoothStep(-vertAnim.angleRange, vertAnim.angleRange,
                        Mathf.PingPong(loopCount / 25f * vertAnim.speed, length: 1f));

                    var jitterOffset = new Vector3(Random.Range(minInclusive: -.25f, maxInclusive: .25f),
                        Random.Range(minInclusive: -.25f, maxInclusive: .25f), z: 0);

                    matrix = Matrix4x4.TRS(jitterOffset * CurveScale,
                        Quaternion.Euler(x: 0, y: 0,
                            Random.Range(minInclusive: -5f, maxInclusive: 5f) * AngleMultiplier), Vector3.one);
