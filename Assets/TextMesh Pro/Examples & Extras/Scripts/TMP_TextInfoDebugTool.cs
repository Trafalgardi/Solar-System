    ...
    public class TMP_TextInfoDebugTool : MonoBehaviour
    {
        ...
        private void DrawCharactersBounds()
        {
            ...
            for (var i = 0; i < characterCount; i++)
            {
                ...
                var topLeft =
                    m_Transform.TransformPoint(new Vector3(characterInfo.topLeft.x, characterInfo.topLeft.y, z: 0));
                ...
                else
                {
                    ...
                    DrawDottedRectangle(
                        m_Transform.TransformPoint(new Vector3(characterInfo.origin, characterInfo.descender, z: 0)),
                        m_Transform.TransformPoint(new Vector3(whiteSpaceAdvance, characterInfo.ascender, z: 0)), color,
                        size: 4);
                }
                ...
                //Draw Ascent line
                var ascentlineStart = m_Transform.TransformPoint(new Vector3(origin, ascentline, z: 0));
                var ascentlineEnd = m_Transform.TransformPoint(new Vector3(advance, ascentline, z: 0));
                ...
                var capHeightStart = new Vector3(topLeft.x,
                    m_Transform.TransformPoint(new Vector3(x: 0, capline, z: 0)).y, z: 0);
                ...
                var meanlineStart = new Vector3(topLeft.x,
                    m_Transform.TransformPoint(new Vector3(x: 0, meanline, z: 0)).y, z: 0);
                ...
                //Draw Base line
                var baselineStart = m_Transform.TransformPoint(new Vector3(origin, baseline, z: 0));
                var baselineEnd = m_Transform.TransformPoint(new Vector3(advance, baseline, z: 0));

                Handles.color = Color.cyan;
                Handles.DrawDottedLine(baselineStart, baselineEnd, dottedLineSize);

                //Draw Descent line
                var descentlineStart = m_Transform.TransformPoint(new Vector3(origin, descentline, z: 0));
                var descentlineEnd = m_Transform.TransformPoint(new Vector3(advance, descentline, z: 0));

                Handles.color = Color.cyan;
                Handles.DrawDottedLine(descentlineStart, descentlineEnd, dottedLineSize);

                // Draw Origin
                var originPosition = m_Transform.TransformPoint(new Vector3(origin, baseline, z: 0));
                DrawCrosshair(originPosition, 0.05f / m_ScaleMultiplier, Color.cyan);

                // Draw Horizontal Advance
                var advancePosition = m_Transform.TransformPoint(new Vector3(advance, baseline, z: 0));
                ...
                {
                    var style = new GUIStyle(GUI.skin.GetStyle(styleName: "Label"));
                    style.normal.textColor = new Color(r: 0.6f, g: 0.6f, b: 0.6f, a: 1.0f);
                    style.fontSize = 12;
                    ...
                    // Ascent Line
                    labelPosition = m_Transform.TransformPoint(new Vector3(center, ascentline, z: 0));
                    style.alignment = TextAnchor.UpperCenter;
                    Handles.Label(labelPosition, text: "Ascent Line", style);
                    //Handles.Label(labelPosition, "Ascent Line (" + ascentlineMetrics.ToString("f3") + ")" , style);

                    // Base Line
                    labelPosition = m_Transform.TransformPoint(new Vector3(center, baseline, z: 0));
                    Handles.Label(labelPosition, text: "Base Line", style);
                    //Handles.Label(labelPosition, "Base Line (" + baselineMetrics.ToString("f3") + ")" , style);

                    // Descent line
                    labelPosition = m_Transform.TransformPoint(new Vector3(center, descentline, z: 0));
                    Handles.Label(labelPosition, text: "Descent Line", style);
                    //Handles.Label(labelPosition, "Descent Line (" + descentlineMetrics.ToString("f3") + ")" , style);

                    if (characterInfo.isVisible)
                    {
                        // Cap Line
                        labelPosition = m_Transform.TransformPoint(new Vector3(center, capline, z: 0));
                        style.alignment = TextAnchor.UpperCenter;
                        Handles.Label(labelPosition, text: "Cap Line", style);
                        //Handles.Label(labelPosition, "Cap Line (" + caplineMetrics.ToString("f3") + ")" , style);

                        // Mean Line
                        labelPosition = m_Transform.TransformPoint(new Vector3(center, meanline, z: 0));
                        style.alignment = TextAnchor.UpperCenter;
                        Handles.Label(labelPosition, text: "Mean Line", style);
                        //Handles.Label(labelPosition, "Mean Line (" + ascentlineMetrics.ToString("f3") + ")" , style);

                        // Origin
                        labelPosition = m_Transform.TransformPoint(new Vector3(origin, baseline, z: 0));
                        style.alignment = TextAnchor.UpperRight;
                        Handles.Label(labelPosition, text: "Origin ", style);

                        // Advance
                        labelPosition = m_Transform.TransformPoint(new Vector3(advance, baseline, z: 0));
                        style.alignment = TextAnchor.UpperLeft;
                        Handles.Label(labelPosition, text: "  Advance", style);
                    }
                }
            }
        }
            ...
            for (var i = 0; i < m_TextInfo.wordCount; i++)
            {
                ...
                for (var j = 0; j < wInfo.characterCount; j++)
                {
                    ...
                    if (isBeginRegion == false && isCharacterVisible)
                    {
                        ...
                        bottomLeft = new Vector3(currentCharInfo.bottomLeft.x, currentCharInfo.descender, z: 0);
                        topLeft = new Vector3(currentCharInfo.bottomLeft.x, currentCharInfo.ascender, z: 0);
                        ...
                        if (wInfo.characterCount == 1)
                        {
                            ...
                            topLeft = m_Transform.TransformPoint(new Vector3(topLeft.x, maxAscender, z: 0));
                            bottomLeft = m_Transform.TransformPoint(new Vector3(bottomLeft.x, minDescender, z: 0));

                            bottomRight =
                                m_Transform.TransformPoint(new Vector3(currentCharInfo.topRight.x, minDescender, z: 0));
                    ...
                    if (isBeginRegion && j == wInfo.characterCount - 1)
                    {
                        ...
                        topRight = m_Transform.TransformPoint(
                            new Vector3(currentCharInfo.topRight.x, maxAscender, z: 0));
        ...
        private void DrawLineBounds()
        {
            ...
            for (var i = 0; i < lineCount; i++)
            {
                ...
                DrawDottedRectangle(m_Transform.TransformPoint(lineInfo.lineExtents.min),
                    m_Transform.TransformPoint(lineInfo.lineExtents.max), Color.green, size: 4);

                // Draw Ascent line
                var ascentlineStart = m_Transform.TransformPoint(new Vector3(lineBottomLeft, ascentline, z: 0));
                var ascentlineEnd = m_Transform.TransformPoint(new Vector3(lineTopRight, ascentline, z: 0));

                Handles.color = Color.yellow;
                Handles.DrawDottedLine(ascentlineStart, ascentlineEnd, dottedLineSize);

                // Draw Base line
                var baseLineStart = m_Transform.TransformPoint(new Vector3(lineBottomLeft, baseline, z: 0));
                var baseLineEnd = m_Transform.TransformPoint(new Vector3(lineTopRight, baseline, z: 0));

                Handles.color = Color.yellow;
                Handles.DrawDottedLine(baseLineStart, baseLineEnd, dottedLineSize);

                // Draw Descent line
                var descentLineStart = m_Transform.TransformPoint(new Vector3(lineBottomLeft, descentline, z: 0));
                var descentLineEnd = m_Transform.TransformPoint(new Vector3(lineTopRight, descentline, z: 0));
                ...
                if (m_HandleSize < 1.0f)
                {
                    ...
                    style.normal.textColor = new Color(r: 0.8f, g: 0.8f, b: 0.8f, a: 1.0f);
                    style.fontSize = 12;
                    style.fixedWidth = 200;
                    style.fixedHeight = 20;
                    Vector3 labelPosition;
                    ...
                    // Ascent Line
                    labelPosition = m_Transform.TransformPoint(new Vector3(lineBottomLeft, ascentline, z: 0));
                    style.padding = new RectOffset(left: 0, right: 10, top: 0, bottom: 5);
                    style.alignment = TextAnchor.MiddleRight;
                    Handles.Label(labelPosition, text: "Ascent Line", style);

                    // Base Line
                    labelPosition = m_Transform.TransformPoint(new Vector3(lineBottomLeft, baseline, z: 0));
                    Handles.Label(labelPosition, text: "Base Line", style);

                    // Descent line
                    labelPosition = m_Transform.TransformPoint(new Vector3(lineBottomLeft, descentline, z: 0));
                    Handles.Label(labelPosition, text: "Descent Line", style);
                }
            }
        }
        ...
        private void DrawBounds()
        {
            ...
            DrawRectangle(bottomLeft, topRight, new Color(r: 1, g: 0.5f, b: 0));
        }
        ...
        private void DrawTextBounds()
        {
            ...
            DrawRectangle(bottomLeft, topRight, new Color(r: 0f, g: 0.5f, b: 0.5f));
        }
        ...
        private void DrawRectangle(Vector3 BL, Vector3 TR, Color color)
        {
            ...
            Gizmos.DrawLine(new Vector3(BL.x, BL.y, z: 0), new Vector3(BL.x, TR.y, z: 0));
            Gizmos.DrawLine(new Vector3(BL.x, TR.y, z: 0), new Vector3(TR.x, TR.y, z: 0));
            Gizmos.DrawLine(new Vector3(TR.x, TR.y, z: 0), new Vector3(TR.x, BL.y, z: 0));
            Gizmos.DrawLine(new Vector3(TR.x, BL.y, z: 0), new Vector3(BL.x, BL.y, z: 0));
        }
