...
namespace TriangleNet.IO
{
    ...
    internal class DebugWriter
    {
        ...
        public void Start(string session)
        {
            ...
            if (stream != null)
            {
                throw new Exception(message: "A session is active. Finish before starting a new.");
            }
        ...
        private void Finish(string path)
        {
            if (stream != null)
            {
                ...
                {
                    using (var gzStream = new GZipStream(gzFile, CompressionMode.Compress, leaveOpen: false))
                    {
                        var bytes = Encoding.UTF8.GetBytes(header);
                        gzStream.Write(bytes, offset: 0, bytes.Length);
        ...
        private void WriteGeometry(IPolygon geometry) => stream.WriteLine(format: "#!G{0}", iteration++);
        ...
        private void WriteMesh(Mesh mesh, bool skip)
        {
            ...
            // Header line
            stream.WriteLine(format: "#!M{0}", iteration++);
            ...
            if (VerticesChanged(mesh))
            {
                ...
                // Number of vertices.
                stream.WriteLine(format: "{0}", mesh.vertices.Count);

                foreach (var v in mesh.vertices.Values)
                {
                    // Vertex number, x and y coordinates and marker.
                    stream.WriteLine(format: "{0} {1} {2} {3}", v.id, v.x.ToString(nfi), v.y.ToString(nfi), v.label);
                }
            }
            else
            {
                stream.WriteLine(value: "0");
            }

            // Number of segments.
            stream.WriteLine(format: "{0}", mesh.subsegs.Count);
            ...
            foreach (var item in mesh.subsegs.Values)
            {
                ...
                stream.WriteLine(format: "{0} {1} {2} {3}", subseg.seg.hash, p1.id, p2.id, subseg.seg.boundary);
            }
            ...
            // Number of triangles.
            stream.WriteLine(format: "{0}", mesh.triangles.Count);
            ...
            foreach (var item in mesh.triangles)
            {
                ...
                stream.Write(format: "{0} {1} {2} {3}", tri.tri.hash, h1, h2, h3);
                ...
                stream.WriteLine(format: " {0} {1} {2}", n1, n2, n3);
            }
        }
