...
namespace TriangleNet.Meshing
{
    ...
    public static class Converter
    {
        ...
        public static DcelMesh ToDCEL(Mesh mesh)
        {
            ...
            {
                face = new Face(generator: null);
                face.id = t.id;

                faces[t.id] = face;

                map[t.id] = new List<HalfEdge>(capacity: 3);
            }
        ...
        private static List<Otri>[] SetNeighbors(Mesh mesh, ITriangle[] triangles)
        {
            ...
            for (i = 0; i < mesh.vertices.Count; i++)
            {
                ...
                vertexarray[i] = new List<Otri>(capacity: 3);
            ...
            foreach (var item in mesh.triangles)
            {
                ...
                for (var j = 0; j < 3; j++)
                {
                    ...
                    {
                        Log.Instance.Error(message: "Triangle has an invalid vertex index.",
                            info: "MeshReader.Reconstruct()");

                        throw new Exception(message: "Triangle has an invalid vertex index.");
                    }
                }
        ...
        private static void SetSegments(Mesh mesh, Polygon polygon, List<Otri>[] vertexarray)
        {
            ...
            if (mesh.behavior.Poly)
            {
                ...
                foreach (var item in mesh.subsegs.Values)
                {
                    ...
                    sorg = polygon.Segments[i].GetVertex(index: 0);
                    sdest = polygon.Segments[i].GetVertex(index: 1);

                    boundmarker = polygon.Segments[i].Label;

                    if (sorg.id < 0 || sorg.id >= mesh.invertices || sdest.id < 0 || sdest.id >= mesh.invertices)
                    {
                        Log.Instance.Error(message: "Segment has an invalid vertex index.",
                            info: "MeshReader.Reconstruct()");

                        throw new Exception(message: "Segment has an invalid vertex index.");
                    }
                        ...
                        while (notfound && checktri.tri.id != Mesh.DUMMY)
                        {
                            ...
                            if (tmp == checkdest)
                            {
                                ...
                                if (checkneighbor.tri.id == Mesh.DUMMY)
                                {
                                    ...
                                    mesh.InsertSubseg(ref checktri, subsegmark: 1);
                                    hullsize++;
                                }
