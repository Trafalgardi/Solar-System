...
namespace TriangleNet.Meshing.Algorithm
{
    ...
    public class SweepLine : ITriangulator
    {
        ...
        public IMesh Triangulate(IList<Vertex> points, Configuration config)
        {
            ...
            HeapDelete(eventheap, heapsize, eventnum: 0);
            heapsize--;
            ...
            do
            {
                if (heapsize == 0)
                {
                    Log.Instance.Error(message: "Input vertices are all identical.", info: "SweepLine.Triangulate()");

                    throw new Exception(message: "Input vertices are all identical.");
                }
                    ...
                    firstvertex.y == secondvertex.y)
                {
                    if (Log.Verbose)
                    {
                        Log.Instance.Warning(
                            "A duplicate vertex appeared and was ignored (ID " + secondvertex.id + ").",
                            info: "SweepLine.Triangulate().1");
                    }
            ...
            while (heapsize > 0)
            {
                ...
                else
                {
                        ...
                        nextvertex.y == lastvertex.y)
                    {
                        ...
                        {
                            Log.Instance.Warning(
                                "A duplicate vertex appeared and was ignored (ID " + nextvertex.id + ").",
                                info: "SweepLine.Triangulate().2");
                        }
        ...
        private void Heapify(SweepEvent[] heap, int heapsize, int eventnum)
        {
            ...
            while (notdone)
            {
                if (heap[leftchild].ykey < eventy ||
                    (heap[leftchild].ykey == eventy && heap[leftchild].xkey < eventx))
                {
                    smallest = leftchild;
                }
                else
