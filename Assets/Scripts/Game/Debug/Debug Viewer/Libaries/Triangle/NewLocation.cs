...
namespace TriangleNet
{
    ...
    internal class NewLocation
    {
        ...
        private readonly double[] poly1 = new double[100];
        private readonly double[] poly2 = new double[100];
        private readonly double[][] polys = new double[3][];
            ...
            if (almostGood == 1)
            {
                ...
                if (relocated > 0)
                {
                    ...
                    switch (relocated)
                    {
                        case 1:
                            //printf("Relocate: (%f,%f)\n", torg[0],torg[1]);			
                            mesh.DeleteVertex(ref delotri); break;
                ...
                else
                {
                    ...
                    xPetalCtr_1 = xMidOfShortestEdge +
                                  Math.Sqrt(petalRadius * petalRadius - shortestEdgeDist / 4) *
                                  (middleAngleCorner.y -
                                   largestAngleCorner.y) /
                                  Math.Sqrt(shortestEdgeDist);

                    yPetalCtr_1 = yMidOfShortestEdge +
                                  Math.Sqrt(petalRadius * petalRadius - shortestEdgeDist / 4) *
                                  (largestAngleCorner.x -
                                   middleAngleCorner.x) /
                                  Math.Sqrt(shortestEdgeDist);

                    xPetalCtr_2 = xMidOfShortestEdge -
                                  Math.Sqrt(petalRadius * petalRadius - shortestEdgeDist / 4) *
                                  (middleAngleCorner.y -
                                   largestAngleCorner.y) /
                                  Math.Sqrt(shortestEdgeDist);

                    yPetalCtr_2 = yMidOfShortestEdge -
                                  Math.Sqrt(petalRadius * petalRadius - shortestEdgeDist / 4) *
                                  (largestAngleCorner.x -
                                   middleAngleCorner.x) /
                                  Math.Sqrt(shortestEdgeDist);
                    ...
                    if (!neighborNotFound)
                    {
                        ...
                        neighborCircumcenter = predicates.FindCircumcenter(neighborvertex_1, neighborvertex_2,
                            neighborvertex_3,
                    ...
                    if (!neighborNotFound)
                    {
                        ...
                        isCorrect = ChooseCorrectPoint(xMidOfMiddleEdge, yMidOfMiddleEdge, p[3], p[4],
                            myCircumcenter.x, myCircumcenter.y, isObtuse: false /*(isObtuse+1)%2*/);
            ...
            if (almostGood == 1)
            {
                ...
                else
                {
                    ...
                    minangle = Math.Acos((middleEdgeDist + longestEdgeDist - shortestEdgeDist) /
                                         (2 * Math.Sqrt(middleEdgeDist) * Math.Sqrt(longestEdgeDist))) *
                               180.0 /
                               Math.PI;
                    ...
                    x_1 = petal_bisector_x * Math.Cos(alpha) +
                          petal_bisector_y * Math.Sin(alpha) +
                          xPetalCtr -
                          xPetalCtr * Math.Cos(alpha) -
                          yPetalCtr * Math.Sin(alpha);

                    y_1 = -petal_bisector_x * Math.Sin(alpha) +
                          petal_bisector_y * Math.Cos(alpha) +
                          yPetalCtr +
                          xPetalCtr * Math.Sin(alpha) -
                          yPetalCtr * Math.Cos(alpha);

                    // rotate the vector ccw around the petal center			
                    x_2 = petal_bisector_x * Math.Cos(alpha) -
                          petal_bisector_y * Math.Sin(alpha) +
                          xPetalCtr -
                          xPetalCtr * Math.Cos(alpha) +
                          yPetalCtr * Math.Sin(alpha);

                    y_2 = petal_bisector_x * Math.Sin(alpha) +
                          petal_bisector_y * Math.Cos(alpha) +
                          yPetalCtr -
                          xPetalCtr * Math.Sin(alpha) -
                          yPetalCtr * Math.Cos(alpha);
                    ...
                    isCorrect = ChooseCorrectPoint(x_2, y_2, middleAngleCorner.x, middleAngleCorner.y, x_1, y_1,
                        isObtuse: true);
                                ...
                                if (Math.Abs(line_result[0] - 1.0) <= EPS && line_p[0] > 0.0)
                                {
                                    ...
                                    if ((smallestAngleCorner.x - petal_slab_inter_x_first) *
                                        ...
                                        MinDistanceToNeighbor(petal_slab_inter_x_first, petal_slab_inter_y_first,
                                            ref neighborotri) >
                                        MinDistanceToNeighbor(line_inter_x, line_inter_y, ref neighborotri))
                                    {
                                    ...
                                    else
                                    {
                                                ...
                                                largestAngleCorner.x, largestAngleCorner.y, line_inter_x, line_inter_y))
                                        {
                                            ...
                                            if (IsBadTriangleAngle(middleAngleCorner.x, middleAngleCorner.y,
                                                    largestAngleCorner.x, largestAngleCorner.y, line_inter_x,
                                                    line_inter_y))
                                            {
                                                // go back to circumcenter
                                                dxFirstSuggestion = dx;
            ...
            ref double[] newloc)
        {
            ...
            numpoints_p = GetStarPoints(badotri, torg, tdest, tapex, whichPoint: 1, ref points_p);
            ...
            numpoints_q = GetStarPoints(badotri, torg, tdest, tapex, whichPoint: 2, ref points_q);
            ...
            numpoints_r = GetStarPoints(badotri, torg, tdest, tapex, whichPoint: 3, ref points_r);
            ...
            double[] points, ref double[] newloc)
        {
            ...
            for (i = 0; i < numpoints * 2; i = i + 2)
            {
                ...
                for (j = 1; j < 4; j++)
                {
                    ...
                    x_3 = x3 * Math.Cos((Math.PI / 3.0 - alpha) * j) +
                          y3 * Math.Sin((Math.PI / 3.0 - alpha) * j) +
                          petalx[i / 2] -
                          petalx[i / 2] * Math.Cos((Math.PI / 3.0 - alpha) * j) -
                          petaly[i / 2] * Math.Sin((Math.PI / 3.0 - alpha) * j);

                    y_3 = -x3 * Math.Sin((Math.PI / 3.0 - alpha) * j) +
                          y3 * Math.Cos((Math.PI / 3.0 - alpha) * j) +
                          petaly[i / 2] +
                          petalx[i / 2] * Math.Sin((Math.PI / 3.0 - alpha) * j) -
                ...
                for (j = 1; j < 4; j++)
                {
                    ...
                    x_4 = x3 * Math.Cos((Math.PI / 3.0 - alpha) * j) -
                          y3 * Math.Sin((Math.PI / 3.0 - alpha) * j) +
                          petalx[i / 2] -
                          petalx[i / 2] * Math.Cos((Math.PI / 3.0 - alpha) * j) +
                          petaly[i / 2] * Math.Sin((Math.PI / 3.0 - alpha) * j);

                    y_4 = x3 * Math.Sin((Math.PI / 3.0 - alpha) * j) +
                          y3 * Math.Cos((Math.PI / 3.0 - alpha) * j) +
                          petaly[i / 2] -
                          petalx[i / 2] * Math.Sin((Math.PI / 3.0 - alpha) * j) -
                          petaly[i / 2] * Math.Cos((Math.PI / 3.0 - alpha) * j);
        ...
        private bool GetWedgeIntersection(int numpoints, double[] points, ref double[] newloc)
        {
            ...
            for (i = 0; i < numpoints * 2; i = i + 2)
            {
                ...
                for (j = 1; j < line345; j++)
                {
                    ...
                    x_3 = x3 * Math.Cos(alpha / (line345 - 1.0) * j) +
                          y3 * Math.Sin(alpha / (line345 - 1.0) * j) +
                          petalx[i / 2] -
                          petalx[i / 2] * Math.Cos(alpha / (line345 - 1.0) * j) -
                          petaly[i / 2] * Math.Sin(alpha / (line345 - 1.0) * j);

                    y_3 = -x3 * Math.Sin(alpha / (line345 - 1.0) * j) +
                          y3 * Math.Cos(alpha / (line345 - 1.0) * j) +
                          petaly[i / 2] +
                          petalx[i / 2] * Math.Sin(alpha / (line345 - 1.0) * j) -
                ...
                for (j = 1; j < line789; j++)
                {
                    ...
                    x_4 = x3 * Math.Cos(alpha / (line789 - 1.0) * j) -
                          y3 * Math.Sin(alpha / (line789 - 1.0) * j) +
                          petalx[i / 2] -
                          petalx[i / 2] * Math.Cos(alpha / (line789 - 1.0) * j) +
                          petaly[i / 2] * Math.Sin(alpha / (line789 - 1.0) * j);

                    y_4 = x3 * Math.Sin(alpha / (line789 - 1.0) * j) +
                          y3 * Math.Cos(alpha / (line789 - 1.0) * j) +
                          petaly[i / 2] -
                          petalx[i / 2] * Math.Sin(alpha / (line789 - 1.0) * j) -
