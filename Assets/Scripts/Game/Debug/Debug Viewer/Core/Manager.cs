...
namespace Visualization
{
    ...
    public static class Manager
    {
        ...
        public static void CreateVisualElement(Mesh mesh, Vector3 position, Quaternion rotation, Vector3 scale)
        {
            ...
            drawList.Add(new VisualElement(mesh, position, rotation, scale, Visualizer.activeColour,
                Visualizer.activeStyle));
        }
        ...
        private static void Draw(Camera camera)
        {
            if (camera && Time.frameCount == lastFrameInputReceived)
            {
                for (var i = 0; i < drawList.Count; i++)
                {
                    ...
                    var matrix = Matrix4x4.TRS(drawData.position, drawData.rotation, drawData.scale);

                    materialProperties.SetColor(name: "_Color", drawData.colour);
                    var activeMaterial = materials[(int)drawData.style];

                    Graphics.DrawMesh(drawData.mesh, matrix, activeMaterial, layer: 0, camera, submeshIndex: 0,
                        materialProperties);
                }
            }
        }
