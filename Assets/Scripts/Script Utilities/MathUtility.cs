...
public static class MathUtility
{
    ...
    public static bool LinesIntersect(Vector2 a1, Vector2 a2, Vector2 a3, Vector2 a4)
        => (a1.x - a2.x) * (a3.y - a4.y) - (a1.y - a2.y) * (a3.x - a4.x) != 0;
    ...
    public static int SideOfLine(Vector2 a, Vector2 b, Vector2 c)
        => (int)Mathf.Sign((c.x - a.x) * (-b.y + a.y) + (c.y - a.y) * (b.x - a.x));

    /// returns the smallest angle between ABC. Never greater than 180
    public static float MinAngle(Vector3 a, Vector3 b, Vector3 c) => Vector3.Angle(a - b, c - b);
    ...
    public static Vector2 V3ToXZ(Vector3 v3) => new(v3.x, v3.z);

    public static Vector3 V2ToXYZ(Vector2 v2, float yValue) => new(v2.x, yValue, v2.y);
}