...
public class PlanetEffects : PostProcessingEffect
{
    ...
    public List<Material> GetMaterials()
    {
        ...
        if (effectHolders.Count > 0)
        {
            ...
            for (var i = 0; i < effectHolders.Count; i++)
            {
                var effectHolder = effectHolders[i];
                ...
                if (displayOceans)
                {
                    if (effectHolder.oceanEffect != null)
                    {
                        ...
                        var camDstFromCentre = (camPos - effectHolder.generator.transform.position).magnitude;
    ...
    {
        var halfHeight = cam.nearClipPlane * Mathf.Tan(cam.fieldOfView * 0.5f * Mathf.Deg2Rad);
        var halfWidth = halfHeight * cam.aspect;
        var dstToNearClipPlaneCorner = new Vector3(halfWidth, halfHeight, cam.nearClipPlane).magnitude;
    ...
    private void SortFarToNear(Vector3 viewPos)
    {
        for (var i = 0; i < effectHolders.Count; i++)
        {
            var dstToSurface = effectHolders[i].DstFromSurface(viewPos);
            sortDistances.Add(dstToSurface);
        }

        for (var i = 0; i < effectHolders.Count - 1; i++)
        {
            for (var j = i + 1; j > 0; j--)
            {
                if (sortDistances[j - 1] < sortDistances[j])
                {
                    var tempDst = sortDistances[j - 1];
    ...
    public class EffectHolder
    {
        ...
        public float DstFromSurface(Vector3 viewPos) => Mathf.Max(a: 0,
            (generator.transform.position - viewPos).magnitude - generator.BodyScale);
    }
}