...
public class CelestialBodyGenerator : MonoBehaviour
{
    ...
    public void OnShapeSettingChanged() => shapeSettingsUpdated = true;

    public void OnShadingNoiseSettingChanged() => shadingNoiseSettingsUpdated = true;
    ...
    public int PickTerrainRes()
    {
        if (!Application.isPlaying)
        {
            switch (previewMode)
            {
                case PreviewMode.LOD0:         return resolutionSettings.lod0;
                case PreviewMode.LOD1:         return resolutionSettings.lod1;
                case PreviewMode.LOD2:         return resolutionSettings.lod2;
                case PreviewMode.CollisionRes: return resolutionSettings.collider;
            }
        }
    ...
    private float UnscaledOceanRadius => Mathf.Lerp(heightMinMax.x, b: 1, body.shading.oceanLevel);
    ...
    private void HandleGameModeGeneration()
    {
        if (CanGenerateMesh())
        {
            ...
            var terrainHolder = GetOrCreateMeshObject(name: "Terrain Mesh", mesh: null, terrainMatInstance);
            ...
            LogTimer(collisionBakeTimer, text: "Mesh collider");
        }
        else
        {
            Debug.Log(message: "Could not generate mesh");
        }
    ...
    private void HandleEditModeGeneration()
    {
        ...
        if (CanGenerateMesh())
        {
            ...
            if (shapeSettingsUpdated)
            {
                ...
                LogTimer(terrainMeshTimer, text: "Generate terrain mesh");
                DrawEditModeMesh();
            }
            ...
            else if (shadingNoiseSettingsUpdated)
            {
                ...
                body.shading.Initialize(body.shape);
                var shadingData = body.shading.GenerateShadingData(vertexBuffer);
                previewMesh.SetUVs(channel: 0, shadingData);
    ...
    private Vector2 GenerateTerrainMesh(ref Mesh mesh, int resolution)
    {
        ...
        // Set heights
        var heights = body.shape.CalculateHeights(vertexBuffer);
        ...
        {
            var perturbShader = body.shape.perturbCompute;
            var maxperturbStrength = body.shape.perturbStrength * edgeLength / 2;

            perturbShader.SetBuffer(kernelIndex: 0, name: "points", vertexBuffer);
            perturbShader.SetInt(name: "numPoints", vertices.Length);
            perturbShader.SetFloat(name: "maxStrength", maxperturbStrength);
        ...
        mesh.SetTriangles(triangles, submesh: 0, calculateBounds: true);
        mesh.RecalculateNormals(); //

        // Shading noise data
        body.shading.Initialize(body.shape);
        var shadingData = body.shading.GenerateShadingData(vertexBuffer);
        mesh.SetUVs(channel: 0, shadingData);
        ...
        for (var i = 0; i < vertices.Length; i++)
        {
            ...
            crudeTangents[i] = new Vector4(-normal.z, y: 0, normal.x, w: 1);
        }
    ...
    {
        var terrainHolder = GetOrCreateMeshObject(name: "Terrain Mesh", previewMesh, body.shading.terrainMaterial);
    }
    ...
    private bool CanGenerateMesh() => ComputeHelper.CanRunEditModeCompute && body.shape && body.shape.heightMapCompute;
