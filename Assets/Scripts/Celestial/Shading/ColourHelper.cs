...
public static class ColourHelper
{
    ...
    public static Color Random(PRNG random, float satMin, float satMax, float valMin, float valMax)
        => Color.HSVToRGB(random.Value(), random.Range(satMin, satMax), random.Range(valMin, valMax));
    ...
    public static Color RandomSimilar(PRNG random, Color original, float maxHueDelta, float maxSatDelta,
        float maxValDelta)
    {
        float hue, sat, val;
    ...
    public static Color RandomSimilar(PRNG random, Color original)
    {
        ...
        sat += random.Sign() * random.Range(min: 0.2f, max: 0.4f);
        sat = Mathf.Clamp(sat, min: 0.2f, max: 0.8f);
        val += random.Sign() * random.Range(min: 0.2f, max: 0.4f);
        val = Mathf.Clamp(val, min: 0.2f, max: 0.8f);
    ...
    public static Color RandomContrasting(PRNG random, Color original)
    {
        ...
        val = val < 0.5f ? random.Range(val + 0.2f, max: 0.9f) : random.Range(min: 0.1f, val - 0.2f);
